...
module AlphaBeta(computer, extendGame, devolveGame, undo, undo', iterativeDeepening) where
...
import Control.Concurrent.Async (race, wait, withAsync, Async )
  ...
  case ghisto of
    ...
    (Left (sz, is) : hs) ->
      let newBoard  = rvSimpleMove (fst assoc) board sz is
          asc'      = changeTurn assoc
          newCMoves = allMoves (fst asc') newBoard
          newHMoves = H.allMoves (fst asc') newBoard
      in case fst assoc of
        Human    -> Game OnGoing asc' newBoard newCMoves H.NoHMoves lhisto hs Start
        Computer -> Game OnGoing asc' newBoard NoCMoves  newHMoves lhisto hs Start
    (Right (sz, mv) : hs) ->
      let newBoard  = rvJumpMove board sz mv (fst assoc)
          asc'      = changeTurn assoc
          newCMoves = allMoves (fst asc') newBoard
...
iterativeDeepening :: Int -> Game ->  IO Game
iterativeDeepening i g = do 
  ior <- newIORef g
  withAsync (iterative 5 16 g ior) $ \a1 ->
    withAsync (threadDelay i) $ \a2 ->
      void $ race (wait a1) (wait a2)
  backtrack <$> readIORef ior

iterative :: Int -> Int -> Game -> IORef Game -> IO ()
iterative i e g irg
  | i > e     = return ()
  | otherwise = do
    print i
    let newGame = alphaBeta i g
    modifyIORef' irg (const newGame)
